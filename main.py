...
def get_args():
	...
	parser.add_argument("-b", "--batchSize",
						help="input the batch size", type=int, default=10)
	parser.add_argument("-p", "--checkpointPath",
						help="input the path of checkpoint", type=str, default="ckp")
	parser.add_argument("-t", "--testStep",
						help="the step of test", type=int, default=10)
	parser.add_argument(
		"-a", "--saveStep", help="the step of save checkpoint", type=int, default=500)
	parser.add_argument(
		"-n", "--number", help="the number of train image", type=int, default=1882)
...
def train():
	...
	image_gray = tf.placeholder(dtype=tf.float32, shape=[
								batch_size, image_size, image_size, 1], name="image_gray")
	image_color = tf.placeholder(dtype=tf.float32, shape=[
								 batch_size, image_size, image_size, 3], name="image_color")
	...
	with tf.control_dependencies(update_ops):
		D_loss = tl.cost.cross_entropy(
			logitsFake.outputs, logitsReal.outputs, name="loss_d")
		g_gan_loss = tl.cost.cross_entropy(
			1-logitsFake.outputs, 1-logitsReal.outputs, name="g_gan_loss")
		g_vgg_loss = tl.cost.mean_squared_error(
			vgg_fake_img.outputs, vgg_real_img.outputs, is_mean=True)
		g_mse_loss = tl.cost.mean_squared_error(
			net_g.outputs, image_color, is_mean=True, name="g_mse_loss") / (3*224**2)
		...
		G_init_optimizer = tf.train.GradientDescentOptimizer(
			1).minimize(g_mse_loss, var_list=G_var)
		D_optimizer = tf.train.AdamOptimizer(
			lr_v, beta1=beta1).minimize(D_loss, var_list=D_var)
		G_optimizer = tf.train.AdamOptimizer(
			lr_v, beta1=beta1).minimize(G_loss, var_list=G_var)
	...
	with tf.Session() as sess:
		...
		for epoch in range(n_epoch_init):
			...
			for idx in range(0, total, batch_size):
				...
				errG, _ = sess.run([G_loss, G_init_optimizer], feed_dict={
								   image_gray: input_gray, image_color: input_color})
			...
			log = "[*] Epoch: [%2d/%2d] time: %4.4fs, g_loss: %.8f" % (
				epoch, n_epoch_init, time.time() - epoch_time, total_g_loss/n_iter)
			print log
		...
		for epoch in range(n_epoch_init, n_epoch):
			...
			for idx in range(0, total, batch_size):
				...
				print(sess.run([logitsFake.outputs, 1-logitsFake.outputs], feed_dict={
								image_gray: input_gray, image_color: input_color}))
				errD, _ = sess.run([D_loss, D_optimizer], feed_dict={
								   image_gray: input_gray, image_color: input_color})
				errG, _ = sess.run([G_loss, G_optimizer], feed_dict={
								   image_gray: input_gray, image_color: input_color})
			...
			if epoch != 0 and epoch % save_step == 0:
				tl.files.save_npz(net_g.all_params, name="%s/g_%d.npz" %
								  (checkpoint_path, mode), sess=sess)
				tl.files.save_npz(net_d.all_params, name="%s/d_%d.npz" %
								  (checkpoint_path, mode), sess=sess)
