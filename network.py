...
def convWithBN(inputs, channels, shape, is_train, nameid, activiation=tf.nn.leaky_relu):

	conv = tf.layers.conv2d(inputs=inputs, filters=channels, kernel_size=shape[0], 
		strides=shape[1], padding="same", activiation=activiation, name="conv"+nameid)
...
def deconvWithBN(inputs, channels, shape, is_train, nameid, activiation=tf.nn.leaky_relu):
	
	deconv = tf.layers.conv2d_transpose(inputs=inputs, filters=channels, kernel_size=shape[0], 
		strides=shape[1], padding="same", activiation=activiation, name="deconv"+nameid)
...
def network_d(image_input, reuse, is_train, keep_prob):
	...
	with tf.variable_scope('network_d', reuse=reuse) as vs:
		...
		conv = tf.layers.dense(inputs=conv, units=256, activiation=tf.nn.relu, name="dense")
