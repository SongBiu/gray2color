...
def network_g(image_gray, reuse, is_train):
    with tf.variable_scope('network_g', reuse=reuse) as vs:
        ...
        block_1 = tl.layers.BatchNormLayer(block_1, is_train=is_train, act=tf.nn.leaky_relu, name="bn1")

        block_2 = tl.layers.Conv2d(block_1, n_filter=32, filter_size=(3, 3), strides=(2, 2), name="conv2")
        block_2 = tl.layers.BatchNormLayer(block_2, is_train=is_train, act=tf.nn.leaky_relu, name="bn2")

        block_3 = tl.layers.Conv2d(block_2, n_filter=64, filter_size=(3, 3), strides=(2, 2), name="conv3")
        block_3 = tl.layers.BatchNormLayer(block_3, is_train=is_train, act=tf.nn.leaky_relu, name="bn3")

        block_4 = tl.layers.Conv2d(block_3, n_filter=128, filter_size=(3, 3), strides=(2, 2), name="conv4")
        block_4 = tl.layers.BatchNormLayer(block_4, is_train=is_train, act=tf.nn.leaky_relu, name="bn4")

        block_3_d = tl.layers.DeConv2d(block_4, n_filter=64, filter_size=(3, 3), strides=(2, 2), name="deconv3")
        block_3_d = tl.layers.BatchNormLayer(block_3_d, is_train=is_train, act=tf.nn.leaky_relu, name="d_bn3")
        block_3_d = tl.layers.ConcatLayer([block_3_d, block_3], concat_dim=3, name="concat1")

        block_2_d = tl.layers.DeConv2d(block_3_d, n_filter=32, filter_size=(3, 3), strides=(2, 2), name="deconv2")
        block_2_d = tl.layers.BatchNormLayer(block_2_d, is_train=is_train, act=tf.nn.leaky_relu, name="d_bn2")
        block_2_d = tl.layers.ConcatLayer([block_2_d, block_2], concat_dim=3, name="concat2")

        block_1_d = tl.layers.DeConv2d(block_2_d, n_filter=16, filter_size=(3, 3), strides=(2, 2), name="deconv1")
        block_1_d = tl.layers.BatchNormLayer(block_1_d, is_train=is_train, act=tf.nn.leaky_relu, name="d_bn1")
        net = tl.layers.ConcatLayer([block_1_d, block_1], concat_dim=3, name="concat3")
        ...
        output = tl.layers.Conv2d(net, n_filter=3, filter_size=(3, 3), strides=(1, 1), act=tf.nn.tanh, name="img_out")
        return output
